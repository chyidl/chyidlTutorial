...
class ChyiEmail():
    ...
    def __init__(self, fromAddr, fromPasswd, toNames, toEmails, subject, msgTemplate=message_Template):
        self._addr = fromAddr
    ...
    def _setupSMTP(self):
        ...
        (self._server_host, self._server_port) = smtp_Dicts[self._addr.split('@')[-1][:-4]]
        try:
            self._server = smtplib.SMTP(host=self._server_host, port=self._server_port)
...
def sendMessage(fromAddr='test@qq.com', fromPasswd='ilsuveijbqelbabh', toNames=['Chyi Test'], toEmails= ['test@gmail.com'], subject="", msgTemplate=""):
    """
    fromAddr:
    fromPasswd:
    ...
    """
    sendEmail = ChyiEmail(fromAddr, fromPasswd, toNames, toEmails, subject, msgTemplate)
