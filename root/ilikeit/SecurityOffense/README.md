...
黑产:
  基于正常的业务产品逻辑，采取非正常或者批量的操作获取利益的行为 
  黑客强调的是技术对抗 
  黑产更看重资源对抗 
  基础安全是防御黑客的技术攻击，避免漏洞 
  业务安全是防御黑产的资源对抗，避免正常业务被攻击 
  业务安全的本质其实就是资源层次上的对抗 
...
风控系统:
  从业务数据中挖掘出黑产行为的数据分析系统
  - 产品方案属于事前的防控，是从根本上提高黑产操作的成本，风控系统属于事中的防控，是在监测到黑产行为时进行拦截
  - 前端SDK - 可以采集各类前端数据、手机型号、硬件类型

规则引擎:
  1.正确的工作模式 
    - 同步
      - 用户输入用户名和密码需要先经过规则引擎的判
    - 异步  
    - - 用户开始可以正常登陆，登陆后才交给规则引擎判定，如果最终确定是黑产，则会封号或者踢出
    - 离线 
通过Redis完成实时计算 
通过Flink完成异步计算 
通过Hive完成离线计算 
...
网络安全中的机器学习:
  - 机器学习实际上是对人为经验的一种总结，并不具备创新的能力，所以总重对抗效果的好坏还是取决于人的对抗能力 
    - 无监督学习
      - 无监督学习的目的是挖掘数据的分布特征，包括数据的聚集特征(聚类特征 是通过聚类算法获得) 分布规律(离群点:  通过时序算法获得)
      - 通过无监督学习，可以发现整体数据中的异样情况;相比人工分析全量数据，无监督学习能够大大提升风控效率
    - 有监督学习
      - 有监督学习的基础是标签数据

数据和特征决定了机器学习的上限，而模型和算法只是逼近这个上限而已 
  对比传统的IP，手机号，设备指纹具有唯一性高稳定性和信息丰富
  1. 设备重置之后，保持设备指纹不变 
  2. 设备更新后，保持设备指纹不变 
  - 软件ID:
    - IOS设备的IDFA，IDFV，Android设备的IMEI，MAC 
