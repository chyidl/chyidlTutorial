...
持续交付的意义:
    1. 效率的提升
    2. 质量保障[在持续交付的过程中进行代码扫描、单元测试、集成测试]
    3. 安全可靠[自动化可以避免误操作的隐患]
    4. 团队协作[让各个团队之间协作更加紧密和紧凑]
    5. 流程更加透明[]

持续交付流程:
    研发人员(Push代码) -> 扫描单测集成测试 -> 结果反馈 -> 创建版本 -> 打包部署(测试) -> 触发[自动化测试平台｜性能平台|项目测试人员｜QA审批] -> 部署[生产环境]

研发阶段:
    分支管理模式 Master 分支 ｜ Test分支 ｜ Feature 分支 | hotfix
        Git Tag: -- 源码
        Packages: -- 源码 + 代码依赖 
        Images: -- 源码 + 代码依赖 + 环境依赖
...
部署模型演进:
    ...
    虚拟机-单机单应用部署       --  
    容器  -单机单应用          -- 
        1. Group: 一组暴露同一服务的集合,对于单机单应用可以简单理解为一组机器
        2. Push|Pull: Group中的某个成员是否可以接收流量和请求，流量可能来自SLB(Server Load Balancer: 是对多台云服务器进行流量分发的负载均衡服务)或者消息系统推送的消息
        3. 生产Group: 是指生产环境Group中第一台被发布验证的机器
        4. 应用初始化、预热、加载数据
        5. 分批: 将同一Group分成多个批次进行滚动部署、减少线上变更对于线上的影响
        6. 降级: 
```
