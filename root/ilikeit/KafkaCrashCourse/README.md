...
Running Apache Kafka as a Linux Service
---------------------------------------
```
Writing System Service Config
    "/etc/systemd/system/" creating and writing two different files containing both our Zookeeper service and Kafka Broker service.

# Change directory into system folder holding services
$ cd /etc/systemd/system

# Create Zookeeper service as root and enter Zookeeper config.
...
User=root
Group=root
ExecStart=/root/kafka_2.13-2.7.0/bin/zookeeper-server-start.sh /root/kafka_2.13-2.7.0/config/zookeeper.properties
ExecStop=/root/kafka_2.13-2.7.0/bin/zookeeper-server-stop.sh

[Install]
WantedBy=multi-user.target

# Create Kafka Broker service as root and enter Kafka Broker config
...
Type=simple
User=root
Group=rootExecStart=/root/kafka_2.13-2.7.0/bin/kafka-server-start.sh /root/kafka_2.13-2.7.0/config/server.properties
ExecStop=/root/kafka_2.13-2.7.0/bin/kafka-server-stop.sh
...
$ sudo systemctl is-active zookeeper.service
...
# Check list of current brokers using Kafka shell script
$ /root/kafka_2.13-2.7.0/bin/zookeeper-shell.sh localhost:2181 <<< "ls /brokers/ids"
```
